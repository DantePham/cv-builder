...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addExperience(); // Add first experience field
    addEducation(); // Add first education field
    addCertification(); // Add first certification field
    addLanguage(); // Add first language field
    addReference(); // Add first reference field
});
...
function setupTemplateSelection() {
    ...
    templateCards.forEach(card => {
        card.querySelector('.btn-select').addEventListener('click', function() {
            ...
            // Chuyển sang trang template riêng
...
function validateForm() {
    const required = ['fullName', 'title', 'email', 'phone'];
...
function generateElegantCV() {
    const { personal, objective, profile, technicalSkills, experience, education, certifications } = cvData;
    ...
    return `
        <div class="cv-elegant">
            <div class="cv-header">
                ...
                <div class="cv-contact" style="margin-top: 20px; display: flex; justify-content: center; gap: 30px; flex-wrap: wrap;">
                        ...
                        <div class="main-section">
                            ...
                            ${education.map(edu => `
                                <div class="cv-item" style="margin-bottom: 20px;">
                                    ...
                                    <div style="color: #666; margin-top: 5px;">${edu.school}</div>
                                    <div style="color: #888; font-style: italic; margin-top: 3px;">${edu.period}</div>
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
                        ...
                        <div class="main-section">
                            ...
                            ${experience.map(exp => `
                                <div class="cv-item" style="margin-bottom: 20px;">
                                    ...
                                    <div style="color: #666; margin-top: 5px;">${exp.company}</div>
                                    <div style="color: #888; font-style: italic; margin-top: 3px;">${exp.period}</div>
                                    ${exp.description ? `<p style="margin-top: 10px; color: #555;">${exp.description}</p>` : ''}
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
...
function generateModernCV() {
    const { personal, objective, technicalSkills, experience, education } = cvData;
    return `
        <div class="cv-modern">
            <div class="cv-header">
                <h1 class="cv-name">${personal.fullName || 'YOUR NAME'}</h1>
...
function loadFromStorage() {
    ...
    if (saved) {
        ...
        if (selectedTemplate) {
            const card = document.querySelector(`[data-template="${selectedTemplate}"]`);
